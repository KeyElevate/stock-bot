...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('s-u')
    .setDescription('Request stock by searching all files for a custom URL match')
    .addStringOption((option) =>
      option
        .setName('service')
        .setDescription('Search term (e.g., netflix, spotify, custom-domain)')
        .setRequired(true)
    ),

  async execute(interaction) {
    const searchTerm = interaction.options.getString('service').toLowerCase().replace(/[^a-z0-9-_]/g, '');
    const userId = interaction.user.id;

    if (!searchTerm) {
      return interaction.reply({
        embeds: [errorEmbed({ title: 'Invalid Search', description: 'Please provide a valid search term.' })],
        ephemeral: true,
      });
    }
